...
export default function Animation() {
    ...
    // 2. dist() function ko pura replace kar do isse (sabse best version)
    // const dist = (px: number, py: number) => {
    //   const w = 0.14   // thickness perfect bold aur smooth ke liye

    //   // Left half of heart (circle)
    //   const leftCircle = Math.sqrt((px + 0.45) ** 2 + (py - 0.25) ** 2) - 0.55

    //   // Right half of heart (circle)
    //   const rightCircle = Math.sqrt((px - 0.45) ** 2 + (py - 0.25) ** 2) - 0.55

    //   // Bottom V shape (two slanted lines)
    //   const bottomLeft = g(px, py, -0.85, -0.15, 0, -0.95, w)
    //   const bottomRight = g(px, py, 0.85, -0.15, 0, -0.95, w)

    //   // Top curves (using circles)
    //   const top = Math.min(leftCircle, rightCircle)

    //   // Combine top + bottom
    //   return Math.min(top, Math.min(bottomLeft, bottomRight)) - w * 0.5
    // }
    const dist = (px: number, py: number) => {
        // Better heart SDF for particle fields (refined lobes)
        let x = px * 1.25;
        let y = py * 1.35;

        // Slight upward shift so bottom looks sharp
        y += 0.18;

        // Heart formula tuned for symmetry
        const a = x * x + y * y - 1;

        // Softening + bulge to fix upper curves
    ...
    useEffect(() => {
        const modes: ColorMode[] = ["default", "sapphire", "gold"]
        let index = 0

        const interval = setInterval(() => {
            index = (index + 1) % modes.length
            setColorMode(modes[index])
        }, 500)
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            index = (index + 1) % modes.length
            setColorMode(modes[index])
        }, 500) // har 4 second mein change (2 sec chahiye toh 2000 kar dena)
    ...
    useEffect(() => {
        ...
        // Use actual displayed size (responsive)
        ...
        const animate = (timestamp: number) => {
            ...
            for (let j = 0; j < particleCount; j++) {
                ...
                if (colorModeRef.current === "sapphire") {
                    ...
                    // Default mode — pure bright white with glow (header text ke saath perfect dikhega)
                    r = 7.0   // pehle 4 tha, ab bright
                    g = 7.0
                    b = 9.0   // thoda bluish white glow ke liye
                }
        ...
        const handleResize = () => {
            ...
            const canvas = canvasRef.current;
            const { camera, renderer, points } = sceneRef.current;
            ...
            // Auto scale heart
            let scale = Math.min(width / 1600, height / 800);

            // Mobile boost
            if (width < 600) {
                scale *= 5.2;   // 👈 mobile par heart 2.2x bada
            }
            ...
            sceneRef.current.points.scale.set(scale, scale, scale);
